...
import Config.Adapter (aspectFromElevation', cropThenAverageRasters', cropThenUnionRasters', residentialProximity', slopeFromElevation', vectorProximityFromFiles')
import Data.Aeson (FromJSON (parseJSON), Options (sumEncoding), SumEncoding (TaggedObject, UntaggedValue), ToJSON (toEncoding), Value (Object, String), defaultOptions, genericParseJSON, genericToEncoding, (.:))
...
data RequireConfig = RequireConfig
  { r_name   :: String
  , r_inputs :: [String]
  , r_output :: String
...
data InputConfig = PathConfig String | RequireOutputConfig String
  deriving (Generic, Show)
